...
function decodeUplink(input) {
    return decodeLoraStripsUplink(input.fPort, input.bytes);
}
...
function decodeDownlink(input) {
    return decodeLoraStripsDownlink(input.fPort, input.bytes);
}
...
function encodeDownlink(input) {
    const result = encodeLoraStripsDownlink(input.message);
    return { bytes: result.data, fPort: result.port };
}
